...
describe("decide", () => {
  ...
  //     ① 两个额度都不够 → 报哪个
  //        IMPORTANT: 期望值变了 —— 阶段 1 是 tool-runs，现在是 model-calls
  //          （模型预算在轮首扣，工具预算只能在 continue 之后扣；
  //           见 docs/decisions/0011-use-case-orchestration.md §④）
  //     ② 预算用光 + completed → 仍然是 done
  //        这条在 decide 这一层已经恒真（它看不到预算），但这条规则本身还活着，
  ...
  describe("要调工具，额度够 → continue", () => {
    it.each<{
      ...
      // IMPORTANT: 这张表现在只有 toolCount 一列还有鉴别力：
