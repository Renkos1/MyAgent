...
/** NUL 字节。TRAP: 写成转义，别在源码里放裸字节 —— shell 和编辑器都会出问题。 */
const NUL = "\u0000";
...
describe("resolveInsideRoot 的性质", () => {
  // SAFETY: 这是整个模块存在的理由 —— CWE-22 路径穿越
  //
...
// ══════════════════════════════════════════════════════════════
// 三条"不成立"的性质 —— 都是既定决定的直接后果，不是 bug。
// 写下「这条不成立、因为哪条决定」，和写下成立的性质一样有价值：
// 它把「我们知道这里不闭合」和「我们没想到」区分开。
// @see docs/decisions/0008-path-resolution.md §③（尾斜杠保留）、§⑥（绝对路径一律拒绝）
describe("不幂等 / 往返不闭合 —— 尾斜杠保留和绝对路径拒绝的代价", () => {
  it("成功的结果再喂回去，必然被拒为 absolute", () => {
    fc.assert(
      fc.property(candidate, (c) => {
        const first = resolveInsideRoot(ROOT, c);
        // NOTE: fc.pre 是断言函数 —— 它同时做"跳过这个用例"和"收窄类型"两件事，
        // 所以下面不需要再写 if (!first.ok) return —— 写了 lint 会报恒假。
        fc.pre(first.ok);
        // 结果是绝对路径，而「绝对路径一律拒绝，即使指向 root 内部」
