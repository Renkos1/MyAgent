...
⑦ 空串算不算「仓库根」
   选择   ★拒绝★，kind: "empty"
   理由   实测 posix.resolve("/repo", "") === posix.resolve("/repo", ".") === "/repo"
          —— 空串和 "." 在 path 那一层是同一个东西。但意图不同：
          "." 是「就是仓库根」，"" ★几乎总是上游拼串拼漏了★。
          让它们返回同一个值，等于把一个 bug 静默成一次合法调用
   代价   想表达「仓库根」必须写 "."，不能靠空串
   反悔   若改成「空串等同于 .」：删掉 empty 这个 kind，同时 §① 要扩写成两种写法

⑧ 含 NUL 字节的路径
   选择   ★拒绝★，kind: "nul-byte"，且必须在任何 path API 之前判
   理由   实测 fs 收到含 U+0000 的路径会抛 ERR_INVALID_ARG_VALUE。
          那是★异常★，会穿过整条 Result 链路直达边界层 ——
          领域层的契约是「失败是返回值」，不能留这么一个例外
   代价   多一个 kind，调用方多一个分支
   NOTE   path.posix.resolve 自己★不会★对 NUL 报错，它只做字符串运算。
          所以这一条挡的是下游的 fs，不是 path
```

> ⑦⑧ 是 2026-09 补记的：两条决定一直只活在 `path.ts` 的分支里，ADR 漏记了。
> ★这不是决定变了，是记录漏了★，所以补进原文而不是另开一份作废它。
> 编号追加在末尾，不插进中间 —— `path.ts:66` 引用着 §③，重排会让它指错。
